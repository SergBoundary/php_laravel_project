            ...
            <div class="col-md-10">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('acc.log-accrual-errors.store') }}">
                            @csrf
                            @php
...
>                                        <select name='personal_card_id' value='personal_card_id' id='personal_card_id' type='text' placeholder="Работник" class="form-control" title='Работник' required>
                                ...
                                <div class='form-group col-md-10'>
                                    ...
                                    <input name='message' id='message' type='text' maxlength="50" class="form-control" title='Сообщение об ошибке'>
                                </div>
                                <div class='form-group col-md-10'>
                                    <label for='error_type'>Статус ошибки</label>
                                    <input name='error_type' id='error_type' type='text' maxlength="50" class="form-control" title='Статус ошибки'>
                                </div>
                                <div class='form-group col-md-10'>
                                    ...
                                    @if(session('success'))
                                        <a class='btn btn-outline-secondary' style="margin-left: 10px;" href="{{ route('acc.log-accrual-errors.index') }}">{{ __('Закрыть') }}</a>
                                    @else
                                        <a class='btn btn-outline-secondary' style="margin-left: 10px;" href="{{ route('acc.log-accrual-errors.index') }}">{{ __('Отмена') }}</a>
                                    @endif
                                </div>
                            </div>
                        </form>
