            ...
            <div class="col-md-10">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('hr.insurance-certificates.store') }}">
                            @csrf
                            @php
...
>                                        <select name='personal_card_id' value='personal_card_id' id='personal_card_id' type='text' placeholder="Работник" class="form-control" title='Работник' required>
                                ...
                                <div class='form-group col-md-10'>
                                    ...
                                    <input name='certificate_series' id='certificate_series' type='text' maxlength="50" class="form-control" title='Серия свидетельства'>
                                </div>
                                <div class='form-group col-md-10'>
                                    <label for='certificate_number'>Номер свидетельства</label>
                                    <input name='certificate_number' id='certificate_number' type='text' maxlength="50" class="form-control" title='Номер свидетельства'>
                                </div>
                                <div class='form-group col-md-10'>
                                    <label for='insurance_fee'>Сумма взноса</label>
                                    <input name='insurance_fee' id='insurance_fee' type='text' maxlength="50" class="form-control" title='Сумма взноса'>
                                </div>
                                <div class='form-group col-md-10'>
                                    <label for='certificate_expiry'>Истечение срока действия</label>
                                    <input name='certificate_expiry' id='certificate_expiry' type='text' maxlength="50" class="form-control" title='Истечение срока действия'>
                                </div>
                                <div class='form-group col-md-10'>
                                    ...
                                    @if(session('success'))
                                        <a class='btn btn-outline-secondary' style="margin-left: 10px;" href="{{ route('hr.insurance-certificates.index') }}">{{ __('Закрыть') }}</a>
                                    @else
                                        <a class='btn btn-outline-secondary' style="margin-left: 10px;" href="{{ route('hr.insurance-certificates.index') }}">{{ __('Отмена') }}</a>
                                    @endif
                                </div>
                            </div>
                        </form>
