...
class CreateMigrationStatusesTable extends Migration {
    ...
    public function up() {
        ...
        Schema::create('migration_statuses', function (Blueprint $table) {
            ...
            $table->date('submitted')->nullable(); // Дата подачи документов в орган легализации пребывания
            $table->date('incomplete')->nullable(); // Дата требования подать недостающие документы
            $table->string('decision_number', 50)->nullable(); // Номер решения о легализации пребывания
            $table->date('decision_date')->nullable(); // Дата выдачи решения о легализации пребывания
            $table->date('date_opening')->nullable(); // Дата открытия пребывания в стране
            $table->date('date_closing')->nullable(); // Дата закрытия пребывания в стране
